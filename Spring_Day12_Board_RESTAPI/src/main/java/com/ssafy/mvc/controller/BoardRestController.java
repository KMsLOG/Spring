...
//	}
	...
	public ResponseEntity<?> write(@ModelAttribute Board board){
		...
		// mapper에서 keyProperty="id" useGeneratedKeys="true" 때문에
		int id = board.getId(); // id가 저장됨
		// 입력한 board 자체를 넘김
		// board를 넘기기 보다는 id 같은거를 넘겨서
		// 프론트에서 detail로 요청을 보내면 좋겠는데
